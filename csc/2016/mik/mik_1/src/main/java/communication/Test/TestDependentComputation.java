 ...
 */
public class TestDependentComputation {
    public static void main(String[] args) {

        Client client = new Client("client4", "localhost", 10918);

        try {
            client.SubmitTask(new Task(buildDependency(1), buildValue(22), buildDependency(1), buildValue(239), (long) 1e9));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    private static Protocol.Task.Param buildDependency(int dependentTaskId) {
