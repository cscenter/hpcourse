...
public class LockFreeSetImpl<T extends Comparable<T>> implements LockFreeSet<T> {
    ...
    public boolean add(T value) {
        while (true) {
            ...
            } else if (last_not_deleted.next.compareAndSet(last_not_deleted_next, new_node, tail_mark, false)) {
                size.incrementAndGet();
                return true;
            }
        }
    ...
    public boolean remove(T value) {
        while (true) {
            ...
            if (pred.getReference() == null) {
                ...
                Node next_node = curr_node.next.getReference();
                if (pred_node.next.compareAndSet(curr_node, next_node, false, false)) {
                    size.decrementAndGet();
                    return true;
                }
            }
    ...
    public boolean contains(T value) {
        while (true) {
            ...
            curr = head;
            while (curr.getReference() != null) {
                if (curr.getReference().value.compareTo(value) == 0 && !curr.isMarked()) {
                    return true;
                }
                curr = curr.getReference().next;
            }
